...
=====================================================
- Test Model: moonshotai/kimi-k2:free (via OpenRouter)
...
# OpenRouter API key for test model (can use free models)
...
# Create separate clients for test model (OpenRouter) and judge model (OpenAI)
# OpenRouter requires HTTP-Referer header (or X-Title) - using default headers
# For free models, you may also need to configure data policy in OpenRouter settings:
# https://openrouter.ai/settings/privacy
test_client = AsyncOpenAI(
    api_key=openrouter_api_key,
    base_url="https://openrouter.ai/api/v1",
    default_headers={
        "HTTP-Referer": "https://github.com/yourusername/yourrepo",  # Optional: Update with your repo URL
        "X-Title": "DS680 Evaluation",  # Optional: App name
        "X-Data-Policy": "allow"  # Required for free models - allows data to be used for training
    }
)
judge_client = AsyncOpenAI(api_key=openai_api_key)

# Config
# Test model via OpenRouter - using free model
...
async def get_model_response(prompt: str) -> str:
    """Get response from model being evaluated (via OpenRouter)."""
